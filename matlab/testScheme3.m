...
% and create new lists of unprocessed blocks
...
%       if Bloczki(ind_).type == 'b',  % ignore mixed blocks
      if (Bloczki(ind_).type == 'b') | (Bloczki(ind_).type == 'm'),
        ...
        if Bloczki(current_Blocks(ind)).number_of_outputs > 0, 
          ...
          if Bloczki(ind_).output_order < Bloczki(ind_).inputs_orders(ind_input)
            ...
            Bloczki = move_output_block(Bloczki, ind_, Bloczki(ind_).inputs_orders(ind_input));
          end
        end
...
DrawBloczki(Bloczki, kolory)

Bloczki = minimize_distance(Bloczki);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function DrawBloczki(Bloczki, kolory)
...
% 3) convert mixed blocks with no inputs into source blocks
for ind = 1:length(Bloczki),
  if Bloczki(ind).type == 'm',
    if Bloczki(ind).number_of_inputs == 0,
      Bloczki(ind).type = 's';
    end
  end
end

% 4) initialize helper fields
for ind = 1:length(Bloczki),
  Bloczki(ind).inputs_orders = -1*ones(1,Bloczki(ind).number_of_inputs);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Bloczki = minimize_distance(Bloczki)

% \todo inputs_orders <- sprawdziæ, czy tego e¿ nie nale¿y aktualizowaæ
...
while (Zmiany > 0),
  Zmiany = 0;
  for ind = 1:length(Bloczki),
%     for ind_out = 1:length(Bloczki(ind).output_blocks),
%       if Bloczki(Bloczki(ind).output_blocks(ind_out)).output_order - Bloczki(ind).output_order > 1,
%         Bloczki(ind).output_order = Bloczki(Bloczki(ind).output_blocks(ind_out)).output_order - 1;
%         Zmiany = Zmiany + 1;
%       end
%     end
    ind_ = Bloczki(ind).output_blocks;
    ind_input = 1 + round((ind_ - floor(ind_)) * 10); % output block input index (1, 2, ...; 0 - not used yet)
    ind_ = floor(ind_);                               % output block index
    if length(ind_) > 0,
      tmp = Bloczki(ind_(1)).output_order;
      for ind_out = 2:length(ind_),
        tmp = min([tmp, Bloczki(ind_(ind_out)).output_order]);
      end
      if tmp - Bloczki(ind).output_order > 1,
        Bloczki(ind).output_order = tmp - 1;
        Zmiany = Zmiany + 1;
      end
    end
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Bloczki, ind_przetworzone] = move_output_block(Bloczki, indeks, new_output_order, ind_przetworzone)

if nargin == 3
  ind_przetworzone = [];
end

if length(find(ind_przetworzone == indeks)) > 0,
  return;
end
ind_przetworzone = [ind_przetworzone, indeks];
  
korekta = new_output_order - Bloczki(indeks).output_order;
Bloczki(indeks).output_order = new_output_order;

ind_ = Bloczki(indeks).output_blocks;
ind_input_ = 1 + round((ind_ - floor(ind_)) * 10); % output block input index (1, 2, ...; 0 - not used yet)
ind_ = floor(ind_);                               % output block index
for ind_ind = 1:length(ind_),
  ind = ind_(ind_ind);
  ind_input = ind_input_(ind_ind)
  Bloczki(ind).inputs_orders
  if  Bloczki(ind).output_order ~= -1, % not set yet
    if Bloczki(ind).inputs_orders(ind_input) ~= -1,
      Bloczki(ind).inputs_orders(ind_input) = Bloczki(ind).inputs_orders(ind_input) + korekta;

      if Bloczki(ind).output_order < Bloczki(ind).inputs_orders(ind_input)
        [Bloczki, ind_przetworzone] = move_output_block(Bloczki, ind, Bloczki(ind).inputs_orders(ind_input), ind_przetworzone)
%         [Bloczki, ind_przetworzone] = move_output_block(Bloczki, ind, Bloczki(ind).output_order + korekta, ind_przetworzone)
      end
    end
  end
end
